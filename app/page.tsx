...
export default function HomePage() {
  ...
  useEffect(() => {
    if (currentPage === "home" && nextPage !== "projects") {
      ...
      setIsMenuOpen(false)
      // Remettre la sphère en état normal quand on revient à home (sauf si on va vers projects)
      if (!isTransitioning) {
        setSphereScale(1)
        setSphereTranslateY(0)
        setIsSphereTransitioning(false)
      }
    } else if (currentPage === "projects" || nextPage === "projects") {
      setIs3DCentered(false)
      setIsMenuOpen(false)
      // Maintenir la sphère en position horizon sur la page projects ou en transition vers projects
      if (!isTransitioning || nextPage === "projects") {
        setSphereScale(3.5)
        setSphereTranslateY(300)
        setIsSphereTransitioning(false)
      }
    } else {
      setIs3DCentered(false)
  ...
  }, [currentPage, nextPage, isTransitioning])
  ...
  const handlePageChange = (newPage: string, direction?: string) => {
    ...
    if (newPage === "projects" && currentPage === "home") {
      ...
      setSphereScale(3.5) // Agrandit la sphère pour un effet horizon plus immersif
      setSphereTranslateY(300) // Descend la sphère vers le bas pour créer l'horizon
    }
    ...
    // Si on reste sur projects ou si on arrive sur projects, maintenir l'horizon
    if (newPage === "projects" || currentPage === "projects") {
      // Pas de changement pendant la transition projects
    }
