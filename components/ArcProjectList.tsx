...
export default function SphereAlignedArcMenu({
  ...
  useEffect(() => {
    function calculateSphereMetrics() {
      ...
      const listOffset = 120 // Distance entre la liste et le bord de la sphère
  ...
  // Angle pour positionner la liste À GAUCHE de la sphère, pas sur son bord
  const centerAngle = Math.PI + (30 * Math.PI) / 180 // 180° + 30° = plus à gauche que le bord
     ...
     >
       ...
       />
        ...
        <div
          ...
          style={{
            left: sphereCenter.x + Math.cos(Math.PI) * arcRadius - 4,
            top: sphereCenter.y + Math.sin(Math.PI) * arcRadius - 4,
            zIndex: 1000,
          }}
        />
      ...
      <AnimatePresence>
        {visibleProjects.map((project, i) => {
          ...
          const angle = startAngle + i * angleStep
           ...
           // Position de la liste À GAUCHE de la sphère
           // Calcul de la position sur l'arc virtuel à gauche de la sphère
           const arcX = sphereCenter.x + Math.cos(angle) * arcRadius
           const arcY = sphereCenter.y + Math.sin(angle) * arcRadius
           
           // Position du texte : aligné à droite pour que la FIN touche l'arc virtuel
           // Le texte s'étend vers la gauche depuis ce point
           const x = arcX // La FIN du texte touche l'arc à gauche de la sphère
           const y = arcY - 12 // Centrage vertical du texte
          ...
          return (
            <motion.div
              ...
              initial={{ opacity: 0, scale: 0.8 }}
                 ...
                 opacity,
                 scale,
                 x: x, // Position exacte de la FIN du texte sur l'arc
