...
export default function SphereAlignedProjectList({
  ...
  const createProjectPositions = () => {
    ...
    // Distance de la liste par rapport à la surface de la sphère (responsive)
    const viewportWidth = window.innerWidth
    const listOffset = viewportWidth < 768 ? 70 : 90 // Plus de distance pour l'effet d'alignement
    const arcRadius = radius + listOffset
    
    // Arc vertical plus étendu pour un effet plus prononcé
    const totalArcAngle = viewportWidth < 768 ? Math.PI * 0.5 : Math.PI * 0.7 // Plus large pour plus d'effet
    ...
    >
      ...
      {process.env.NODE_ENV === 'development' && (
        <>
          ...
          <div
            ...
            style={{
              left: sphereMetrics.centerX - sphereMetrics.radius - (window.innerWidth < 768 ? 70 : 90) - 4,
              top: sphereMetrics.centerY - 4,
              zIndex: 1000,
            }}
            title="Position arc - fin du texte alignée ici"
          />
        </>
      )}
        ...
        {projectPositions.map((item, index) => (
          <motion.div
            ...
            style={{
              ...
              transform: "translateX(-100%)", // Alignement parfait du bord droit sur l'arc
              textShadow: item.isSelected ? "0 2px 8px rgba(0,0,0,0.3)" : "none",
              // Petite marge pour éviter que le texte touche exactement l'arc
              paddingRight: "4px",
            }}
      ...
      {projects.length > maxVisible && (
        <div 
          ...
          style={{
            left: sphereMetrics.centerX - sphereMetrics.radius - (window.innerWidth < 768 ? 140 : 160),
