...
export default function SphereAlignedProjectList({
  ...
  // Calcul simplifié des métriques de la sphère avec centrage CSS parfait
  useEffect(() => {
    function calculateSphereMetrics() {
      const viewportHeight = window.innerHeight
      const viewportWidth = window.innerWidth

      // Shader original : vec2 screenP = (FC.xy * 2.0 - r) / r.y;
      // baseRadius = 0.9 en coordonnées normalisées
      const baseRadius = 0.9
      
      // Calcul simplifié grâce au centrage CSS parfait
      const sphereRadiusPixels = baseRadius * viewportHeight / 2
      
      // Centre parfait grâce au CSS transform: translate(-50%, -50%)
