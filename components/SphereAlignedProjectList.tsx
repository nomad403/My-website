...
export default function SphereAlignedProjectList({
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      // Si on a moins de projets que maxVisible, on centre différemment
      if (projects.length <= maxVisible) {
        // Tous les projets sont visibles, on calcule pour centrer selected
        const totalVisible = projects.length
        const middleIndex = Math.floor((totalVisible - 1) / 2)
        const offset = selected - middleIndex
        setFirstVisible(Math.max(0, offset))
      } else {
        // Logique normale pour plus de projets
