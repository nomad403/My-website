...
export default function SphereAlignedProjectList({
  ...
  // Calcul exactement synchronisé avec le shader UV corrigé
  useEffect(() => {
    function calculateSphereMetrics() {
      const viewportHeight = window.innerHeight
      const viewportWidth = window.innerWidth

      // Reproduction exacte du nouveau shader UV :
      // vec2 screenP = (vUv - 0.5) * 2.0;
      // screenP.x *= r.x / r.y;
      // baseRadius = 0.9 en coordonnées normalisées
      const baseRadius = 0.9
      
      // Calcul du rayon avec correction d'aspect ratio identique au shader
      // Le rayon est basé sur la dimension Y (hauteur) comme référence
      const sphereRadiusPixels = baseRadius * viewportHeight / 2
      
      // Centre parfait (identique au shader avec vUv)
  ...
  // Calcul de l'arc tangent à la sphère côté gauche (optimisé)
  const createProjectPositions = () => {
    if (!sphereMetrics.radius) return []

    const { centerX, centerY, radius } = sphereMetrics
    
    // Distance de la liste adaptée à la taille de l'écran
    const viewportWidth = window.innerWidth
    const listOffset = viewportWidth < 768 ? 60 : 80 // Distance réduite sur mobile
    const arcRadius = radius + listOffset
    
    // Arc vertical centré sur la sphère (côté gauche) - adaptatif
    // Angle total pour une distribution harmonieuse qui suit la courbure
    const totalArcAngle = viewportWidth < 768 
      ? Math.PI * 0.4  // 72 degrés sur mobile (arc plus serré)
      : Math.PI * 0.6  // 108 degrés sur desktop
    const angleStep = visibleProjects.length > 1 ? totalArcAngle / (visibleProjects.length - 1) : 0
    
    // Angle de départ pour centrer parfaitement l'arc verticalement
  ...
  return (
    ...
    >
        ...
        <>
          ...
          />
           ...
           <div
             ...
             style={{
               left: sphereMetrics.centerX - sphereMetrics.radius - (window.innerWidth < 768 ? 60 : 80) - 4,
               top: sphereMetrics.centerY - 4,
               zIndex: 1000,
             }}
             title="Position arc responsive"
           />
        </>
      )}
          ...
          <motion.div
            ...
            transition={{
              ...
              opacity: { duration: 0.3 },
               ...
               ${
                 item.isSelected
                   ? "text-white font-semibold text-sm sm:text-base md:text-lg drop-shadow-lg"
                   : "text-gray-800 hover:text-orange-600 font-medium text-xs sm:text-sm md:text-base"
               }
      ...
      {projects.length > maxVisible && (
           ...
           style={{
             left: sphereMetrics.centerX - sphereMetrics.radius - (window.innerWidth < 768 ? 110 : 140),
         ...
         >
             ...
             <motion.button
               ...
               className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-gray-800 hover:bg-white/30 transition-all text-xs sm:text-base"
             >
               ↑
             </motion.button>
           )}
