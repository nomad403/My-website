...
export default function SphereAlignedProjectList({
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      const centerIndex = Math.floor(maxVisible / 2) // Index central de la vue (ex: index 3 pour 7 éléments)
      
      if (projects.length <= maxVisible) {
        // Si tous les projets sont visibles, on ajuste firstVisible pour centrer selected
        const offset = centerIndex - Math.floor((projects.length - 1) / 2)
        setFirstVisible(Math.max(0, offset))
      } else {
        // Logique normale : centre selected à l'index centerIndex
  ...
  const createProjectPositions = () => {
    ...
    const arcRadius = radius + listOffset
      ...
      const totalArcAngle = Math.PI * 0.4 // 72 degrés pour un espacement plus compact
    ...
    return visibleProjects.map((project, index) => {
      ...
      const y = centerY + Math.sin(angle) * arcRadius
       ...
       const centerIndex = Math.floor(maxVisible / 2) // Index central (ex: 3 pour 7 éléments)
