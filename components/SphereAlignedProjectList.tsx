...
export default function SphereAlignedProjectList({
  ...
  const handleWheel = (e: WheelEvent) => {
    ...
    if (e.deltaY > 0) {
      // Scroll vers le bas = projet suivant (avec boucle)
      const nextSelected = (selected + 1) % projects.length
      onSelect(nextSelected)
    } else if (e.deltaY < 0) {
      // Scroll vers le haut = projet précédent (avec boucle)
  ...
  return (
    ...
    >
         ...
         {projectPositions.map((item, index) => (
                                               <motion.div
               key={`carousel-${firstVisible}-${index}`}
       ...
       {projects.length > 1 && (
          ...
          >
           <motion.button
             ...
             onClick={() => onSelect((selected - 1 + projects.length) % projects.length)}
           ...
           <motion.button
             ...
             animate={{ opacity: 0.6 }}
             whileHover={{ opacity: 1, scale: 1.1 }}
             onClick={() => onSelect((selected + 1) % projects.length)}
             className="w-8 h-8 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-gray-800 hover:bg-white/30 transition-all"
           >
             ↓
           </motion.button>
