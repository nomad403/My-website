...
export default function SphereAlignedProjectList({
  ...
  const createProjectPositions = () => {
    ...
    return visibleProjects.map((project, index) => {
      ...
      // Position X fixe à droite
      const x = window.innerWidth - 280 // 280px du bord droit
          ...
          >
            ...
            {item.isSelected && (
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="absolute -left-3 top-1/2 transform -translate-y-1/2 w-3 h-3 bg-orange-500 rounded-full shadow-lg border-2 border-orange-300"
              />
            )}
          </motion.div>
        ))}
