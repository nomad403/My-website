...
export default function SphereAlignedProjectList({
  ...
  // Calcul précis des métriques de la sphère basé sur le shader réel
  useEffect(() => {
    function calculateSphereMetrics() {
      const viewportHeight = window.innerHeight
      const viewportWidth = window.innerWidth

      // Reproduction exacte du calcul du shader
      // vec2 screenP = (FC.xy * 2.0 - r) / r.y;
      // baseRadius = 0.9 en coordonnées normalisées
      const baseRadius = 0.9
      
      // Conversion en pixels réels
      // Le rayon en pixels est baseRadius * min(width, height) / 2
      const sphereRadiusPixels = baseRadius * Math.min(viewportWidth, viewportHeight) / 2
      
      // Centre exacte de la sphère (comme dans le shader: screenP = (0,0))
  ...
  const createProjectPositions = () => {
    ...
    const listOffset = 80 // Distance en pixels depuis la surface de la sphère
    const arcRadius = radius + listOffset
    
    // Arc vertical centré sur la sphère (côté gauche)
    // Angle total pour distribuer les projets (en radians)
    const totalArcAngle = Math.PI * 0.6 // 108 degrés total
  ...
  return (
    <div
      ...
      style={{ zIndex: 25 }}
    >
      {/* Repères visuels de debug (à supprimer en production) */}
